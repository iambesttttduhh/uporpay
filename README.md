...
this is merged. There is also a rolling prerelease holding the CI-published build,
[wake-or-lock-debug](https://github.com/iambesttttduhh/uporpay/releases/tag/wake-or-lock-debug), refreshed by pushing a `v*` tag or the *publish* input on the
workflow, if you would rather have release notes next to it.

The repo is private, so either sign in to GitHub in the browser you download from, or fetch it
from a terminal: `gh api repos/iambesttttduhh/uporpay/contents/builds/wake-or-lock-debug.apk -q
.content | base64 -d > wake-or-lock-debug.apk`. Install it directly on the phone, or over USB with
`adb install -r builds/wake-or-lock-debug.apk`.
